...
export function usePeer(userId: string | undefined) {
  ...
  useEffect(() => {
    ...
    newPeer.on('error', (err: any) => {
      ...
      if (type === 'peer-unavailable' || message.includes('Could not connect to peer')) {
        friendlyMessage = "Target user is currently unavailable. They might have closed the app or have a poor connection.";
      } else if (type === 'network' || message.includes('Lost connection')) {
        friendlyMessage = "Peer server connection temporarily dropped. Reconnecting automatically...";
        try {
        ...
        friendlyMessage = "Signaling server connection interrupted. Retrying...";
      ...
      console.warn(`[PeerJS Notice] Type: ${type || 'network'}. Message: ${friendlyMessage}`);
